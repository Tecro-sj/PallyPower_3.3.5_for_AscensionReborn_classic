...
function PallyPower:OnInitialize()
	...
	self:RegisterChatCommand("ppd", "DebugSync")
	self:RegisterChatCommand("ppsync", "ForceSync")
...
function PallyPower:CHAT_MSG_ADDON()
...
function PallyPower:CanControl(name)
	...
	elseif GetNumPartyMembers() > 0 then
		for i = 1, GetNumPartyMembers() do
			if UnitIsPartyLeader("party"..i) and UnitIsUnit("party"..i, "player") then
				isLeader = true
				break
			end
		end
		if not isLeader and UnitIsPartyLeader("player") then
			isLeader = true
		end
	else
		isLeader = true
	...
	return (isLeader or isAssistant or (name==self.player) or (AllPallys[name] and AllPallys[name].freeassign == true))
end
...
function PallyPower:DebugSync()
	self:Print("=== PallyPower Debug Info ===")
	self:Print("Your name: " .. self.player)
	self:Print("Number of paladins in SyncList: " .. #SyncList)
	for i, name in ipairs(SyncList) do
		self:Print(i .. ". " .. name)
	end
	self:Print("All recognized paladins (AllPallys):")
	for name, data in pairs(AllPallys) do
		self:Print("  - " .. name)
	end
	self:Print("Raid size: " .. GetNumRaidMembers())
	self:Print("Party size: " .. GetNumPartyMembers())
end

function PallyPower:ForceSync()
	self:Print("Sending sync request (REQ) to all paladins...")
	self:SendMessage("REQ")
	self:Print("Sending your info (SELF) to all paladins...")
	self:SendSelf()
	self:Print("Sync request sent! Other paladins with PallyPower should respond.")
end
